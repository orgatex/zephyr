...
struct ssd16xx_profile {
	...
	struct ssd16xx_dt_array ctrl1;
	uint8_t vcom;
	uint8_t bwf;
	uint8_t dummy_line;
	uint8_t gate_line_width;
	uint8_t display_off_sequence;

	bool override_vcom;
	bool override_bwf;
	bool override_dummy_line;
	bool override_gate_line_width;
	bool override_display_off_sequence;
};
...
static int ssd16xx_update_display(const struct device *dev)
{
	...
	uint8_t update_cmd = SSD16XX_CTRL2_ENABLE_CLK | SSD16XX_CTRL2_ENABLE_ANALOG |
			 ...
			 const struct display_buffer_descriptor *desc, const void *buf)
{
	const struct ssd16xx_config *config = dev->config;
	const struct ssd16xx_data *data = dev->data;
	const struct ssd16xx_profile *p = config->profiles[SSD16XX_PROFILE_PARTIAL];
	...
	if (partial_refresh) {
		if (p && p->ctrl1.len) {
			err = ssd16xx_write_cmd(dev, SSD16XX_CMD_UPDATE_CTRL1, p->ctrl1.data,
						p->ctrl1.len);
		}
		if (err < 0) {
			return err;
...
static int ssd16xx_set_profile(const struct device *dev, enum ssd16xx_profile_type type)
{
	...
	if (p && p->override_display_off_sequence) {
		LOG_DBG("Setting OFF sequence");
		err = ssd16xx_write_cmd(dev, SSD16XX_CMD_OFF_SEQUNECE, &p->display_off_sequence, 1);
		if (err < 0) {
			return err;
		}
	}
...
#define SSD16XX_PROFILE(n)                                                                         \
	...
	static const struct ssd16xx_profile ssd16xx_profile_##n = {                                \
		...
		.gdv = SSD16XX_ASSIGN_ARRAY(n, gdv),                                               \
		.sdv = SSD16XX_ASSIGN_ARRAY(n, sdv),                                               \
		.ctrl1 = SSD16XX_ASSIGN_ARRAY(n, ctrl1),                                           \
		.vcom = DT_PROP_OR(n, vcom, 0),                                                    \
		.override_vcom = DT_NODE_HAS_PROP(n, vcom),                                        \
		.bwf = DT_PROP_OR(n, border_waveform, 0),                                          \
		.override_bwf = DT_NODE_HAS_PROP(n, border_waveform),                              \
		...
		.gate_line_width = DT_PROP_OR(n, gate_line_width, 0),                              \
		.override_gate_line_width = DT_NODE_HAS_PROP(n, gate_line_width),                  \
		.display_off_sequence = DT_PROP_OR(n, display_off_sequence, 0),                    \
		.override_display_off_sequence = DT_NODE_HAS_PROP(n, display_off_sequence),        \
	};
